...
```{r createTxDBGTF2, eval=FALSE, echo=FALSE}
library(GenomicFeatures)
txMm <- makeTxDbFromGFF(file = "../Course_Materials/references/Mus_musculus.GRCm38.97.gtf", format = "gtf")
```
...
previous two methods are adequate if you can tolerate the short wait each time 
you create the database.

```{r creatTxDBpackage2, eval=FALSE, echo=FALSE}
library(GenomicFeatures)
makeTxDbPackageFromBiomart(version="0.01",
                           destDir = "../Course_Materials/references/",
...
cd ../Course_Materials/references
...
```{r bamFiles, eval=FALSE}
list.files("~/Course_Materials/RNAseq/small_bams/")
```


```{r bamFiles2, echo=FALSE}
list.files("../Course_Materials/small_bams/")
```
...
```{r readReads2, echo=FALSE}
exo <- exonsBy(txMm, "gene") 
generegion <- exo[["ENSMUSG00000022146"]] %>% 
    keepSeqlevels(value = 15, pruning.mode="tidy")

my.reads <- readGAlignments(file="../Course_Materials/small_bams/MCL1.DG.15.sm.bam",
                       param=ScanBamParam(which=generegion))
my.reads
```
...
```{r coveragePlot2, echo=FALSE}
myreg <- exo[["ENSMUSG00000022146"]] %>% 
    GenomicRanges::reduce() %>% 
    flank(width = 1000, both = T) %>% 
    keepSeqlevels(value = 15, pruning.mode="tidy")

bam <- readGappedReads(file="../Course_Materials/small_bams/MCL1.DG.15.sm.bam",
                       param=ScanBamParam(which=myreg), use.names = TRUE)
